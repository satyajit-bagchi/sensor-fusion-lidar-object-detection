...
struct KdTree {
    ...
    void searchHelper(std::vector<float> target, Node* node, int depth,
        ...
        if (node != NULL) {
            bool is_x_minus_in_range =
                node->point[0] >= (target[0] - distanceTol);
            bool is_x_plus_in_range =
                node->point[0] <= (target[0] + distanceTol);
            bool is_y_minus_in_range =
                node->point[1] >= (target[1] - distanceTol);
            bool is_y_plus_in_range =
                node->point[1] <= (target[1] + distanceTol);
            if (is_x_minus_in_range && is_x_plus_in_range &&
                is_y_minus_in_range && is_y_plus_in_range) {
                auto delta_x = node->point[0] - target[0];
                auto delta_y = node->point[1] - target[1];
                float distance =
